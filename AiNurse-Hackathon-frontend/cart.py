...
def search_api(data):
    ...
    try:
        print(data)
        for item in data:
            url = "http://sspl-search.fkh.prv/search_list?device=5&page=1&size=40&app_type=M&token=l59i759i7589o5966589l589958954t&wh=1&pincode=700156&q=" + item["product"] + "%" + item["type"] + "%" + item["dosage"] + "&userType=NEW_USER&userGroup=CUSTOMER"
            # params = {
            #     "device": "5",
            #     "page": "1",
            #     "size": "40",
            ...
            if(length > 0):
                for product in response.json()['products']:
                    integer_value = get_integer(product[49])
                    print("integer value - " + str(integer_value))
                    if integer_value is not None and integer_value > 2000:
                        product_details = {"id": product[23], "name": product[0], "strength": product[30]}
